...
(function ($) {
    ...
    window.VcColLeftIconView = vc.shortcode_view.extend({
       changeShortcodeParams:function (model) {
            var params = model.get('params');
            if (_.isObject(params)) {
				 this.$el.find('.wpb_element_wrapper .columns3').remove('');				
                this.$el.find('.wpb_element_wrapper').append('<div class="columns3"><div class="circle"><div class="topLeftIcon icon-'+params.icon+'"></div></div><div class="content"><h1>'+params.title+'</h1></div><div class="clear"></div>'+params.content+'</div>');   
				/*if ((params.url_value != '' )&&(params.url_caption != '')) {
					this.$el.find('.wpb_element_wrapper').append('<div class="read-more-text"><a href="'+params.url_value+'" >'+params.url_caption+' </a></div>');
				}
				this.$el.find('.wpb_element_wrapper').append('</div>');
				*/
	...
	window.VcColTopIconView = vc.shortcode_view.extend({
       changeShortcodeParams:function (model) {
            var params = model.get('params');
            if (_.isObject(params)) {
				this.$el.find('.wpb_element_wrapper .columns1').remove('');
				
				var iconwrapper_css;				
				if ((params.icon_bgcolor != '')||(params.icon_color != '')) {
					iconwrapper_css = ' style= " ';
					if (params.icon_bgcolor != '') {
						iconwrapper_css = iconwrapper_css+' background-color: '+params.icon_bgcolor+';  ';
					}
					iconwrapper_css =  iconwrapper_css+' " ';	
				}
				var icon_css;
				if (params.icon_color != '') {
					icon_css = icon_css+' style= " ';
					if (params.icon_color != '') {
						icon_css = icon_css+' color: '+params.icon_color+';  ';
					}
					icon_css = icon_css+' " ';	
				}					
				this.$el.find('.wpb_element_wrapper').append('<div class="columns1"><div class="icon"'+iconwrapper_css+'><div class="topIcon icon-'+params.icon+'" '+icon_css+'></div></div><h1>'+params.title+'</h1>'+params.content+'</div>'); 				
            }
        }
    });
	
	
	window.VcColTopIcon2View = vc.shortcode_view.extend({
       changeShortcodeParams:function (model) {
            var params = model.get('params');
            if (_.isObject(params)) {
				this.$el.find('.wpb_element_wrapper .columns2').remove('');
				
				var iconwrapper_css;				
				if ((params.column_bgcolor != '')||(params.icon_color != '')) {
					iconwrapper_css = ' style= " ';
					if (params.column_bgcolor != '') {
						iconwrapper_css = iconwrapper_css+' background-color: '+params.column_bgcolor+';  ';
					}
					iconwrapper_css =  iconwrapper_css+' " ';	
				}
				var icon_css;
				...
				this.$el.find('.wpb_element_wrapper').append('<div class="columns2"><div class="circle"><div class="topIcon icon-'+params.icon+'"'+icon_css+'></div></div><div class="content"'+iconwrapper_css+'><h1><a href="'+params.url_value+'" >'+params.title+'</a></h1>'+params.content+'<a href="'+params.url_value+'"><span class="wpb_button  '+params.color+' wpb_regularsize" style="margin-bottom:0;">'+params.url_caption+'</span></a></div></div>');			
            }
        }
    });
