...
foreach ( $tab_titles as $tab ) {
    ...
    if(isset($tab_matches[1][0])) {
      ...
      //  if ($icon != '') {
			//$tabs_nav .= '<div class="iconTabTitle icon-'.$icon.'"></div>';
		//	$tabs_nav .= ' 1'.$tab_matches[1][0] .' 2'.$tab_matches[2][0] .' 3'.$tab_matches[3][0] .'4'.$tab_matches[4][0];
		//	$tabs_nav .= '1'.$content;
       //}
...
$tabs_nav .= '</ul><div class="clear"></div>'."\n";
...
$output .= "\n\t\t\t".$tabs_nav;
$output .= "<div class='tab-content-border-wrapper'>";
$output .= "\n\t\t\t".wpb_js_remove_wpautop($content);
$output .= "</div>";
if ( 'vc_tour' == $this->shortcode) {
    $output .= "\n\t\t\t" . '<div class="wpb_tour_next_prev_nav clearfix"> <span class="wpb_prev_slide"><a href="#prev" title="'.__('Previous slide', 'meganews').'">'.__('Previous slide', 'meganews').'</a></span> <span class="wpb_next_slide"><a href="#next" title="'.__('Next slide', 'meganews').'">'.__('Next slide', 'meganews').'</a></span></div>';
}
