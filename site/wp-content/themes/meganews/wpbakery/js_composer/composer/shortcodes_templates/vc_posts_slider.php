...
while ( $my_query->have_posts() ) {
    ...
    // if ( $thumbnail == '' ) $thumbnail = __("No Featured image set.", "meganews");

    // Link logic
    if ( $link != 'link_no' ) {
        if ( $link == 'link_post' ) {
            $link_image_start = '<a class="link_image" href="'.get_permalink($post_id).'" title="'.sprintf( esc_attr__( 'Permalink to %s', 'meganews' ), the_title_attribute( 'echo=0' ) ).'">';
        }
