...
function vc_loop_form_field($settings, $value) {
    ...
    return '<div class="vc-loop">'
        ...
        .'<a href="#" class="button vc-loop-build '.$settings['param_name'].'_button" data-settings="'.rawurlencode(json_encode($settings['settings'])).'">'.__('Build query', 'meganews').'</a>'
        .'<div class="loop-info"><h4>'.__('Saved data').'</h4>'.$loop_info.'</div>'
        .'</div>';
}
    ...
    if(isset($param['options']) && is_array($param['options'])) {
        foreach($param['options'] as $option) {
            ...
            if(is_array($option) && isset($option['value'])) {
                if(in_array($option['value'], $selected_values)) $value[] = $option['action'].$option['name'];
...
class VcLoopSettings {
    ...
    function __construct($value, $settings = array()) {
        $this->parts = array(
            'size' => __('Post Count', 'meganews'),
            'order_by' => __('Order By', 'meganews'),
            'order' => __('Order', 'meganews'),
            'post_type' => __('Post types', 'meganews'),
            'authors' => __('Author', 'meganews'),
            'categories' => __('Categories', 'meganews'),
            'tags' => __('Tags', 'meganews'),
            'tax_query' => __('Taxonomies', 'meganews'),
            'by_id' => __('Individual Posts/Pages', 'meganews')
        );
    ...
    public function parse_order_by($value) {
        return $this->parseDropDown($value, array(
            //array('none', __("None", 'meganews')),
            array('date', __("Date", 'meganews')),
            'ID',
            array('author', __("Author", 'meganews')),
            array('title', __("Title", 'meganews')),
            //'name',
            array('modified', __("Modified", 'meganews')),
            //'parent',
            array('rand', __("Random", 'meganews')),
            array('comment_count', __("Comment count", 'meganews')),
            array('menu_order', __("Menu order", 'meganews')),
            //'meta_value',
            //'meta_value_num',
            //'post__in'
        ));
    ...
    public function parse_order($value) {
        return $this->parseDropDown($value, array(
            array('ASC', __("Ascending", 'meganews')),
            array('DESC', __("Descending", 'meganews'))
        ));
    }
    ...
    public static function buildWpQuery($query) {
 ...
 * @return string
 */
function vc_build_loop_query($query) {
    return VcLoopSettings::buildWpQuery($query);
}
