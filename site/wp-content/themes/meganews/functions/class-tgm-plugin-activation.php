...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function __construct() {
			...
			$this->strings = array(
				'page_title'                      => __( 'Install Required Plugins', 'remarky' ),
				'menu_title'                      => __( 'Install Plugins', 'remarky' ),
				'installing'                      => __( 'Installing Plugin: %s', 'remarky' ),
				'oops'                            => __( 'Something went wrong.', 'remarky' ),
				'notice_can_install_required'     => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'remarky' ),
				...
				'return'                          => __( 'Return to Required Plugins Installer', 'remarky' ),
				'plugin_activated'                => __( 'Plugin activated successfully.', 'remarky' ),
				'complete'                        => __( 'All plugins installed and activated successfully. %1$s', 'remarky' ),
			);
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function _gather_plugin_data() {
			...
			foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
				...
				if ( isset( $plugin['external_url'] ) ) {
					...
					$table_data[$i]['source'] = __( 'External Link', 'remarky' );
				}
				elseif ( isset( $plugin['source'] ) ) {
					/** The plugin must be from a private repository */
					if ( preg_match( '|^http(s)?://|', $plugin['source'] ) )
						$table_data[$i]['source'] = __( 'Private Repository', 'remarky' );
					/** The plugin is pre-packaged with the theme */
					else
						$table_data[$i]['source'] = __( 'Pre-Packaged', 'remarky' );
				}
				/** The plugin is from the WordPress repository */
				else {
					$table_data[$i]['source'] = __( 'WordPress Repository', 'remarky' );
				}

				$table_data[$i]['type'] = $plugin['required'] ? __( 'Required', 'remarky' ) : __( 'Recommended', 'remarky' );

				if ( ! isset( $installed_plugins[$plugin['file_path']] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'remarky' ) );
				elseif ( is_plugin_inactive( $plugin['file_path'] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'remarky' ) );
		...
		public function no_items() {
			...
			printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'remarky' ), admin_url() );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		...
		public function get_columns() {
			...
			$columns = array(
				...
				'plugin' => __( 'Plugin', 'remarky' ),
				'source' => __( 'Source', 'remarky' ),
				'type'   => __( 'Type', 'remarky' ),
				'status' => __( 'Status', 'remarky' )
			);
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				'tgmpa-bulk-install'  => __( 'Install', 'remarky' ),
				'tgmpa-bulk-activate' => __( 'Activate', 'remarky' ),
			);
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( is_wp_error( $activate ) )
					...
					printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'remarky' ), $imploded );
	...
	if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
		...
		class TGM_Bulk_Installer extends WP_Upgrader {
			...
			public function install_strings() {
				...
				$this->strings['no_package']          = __( 'Install package not available.', 'remarky' );
				$this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'remarky' );
				$this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'remarky' );
				$this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'remarky' );
				$this->strings['process_failed']      = __( 'Plugin install failed.', 'remarky' );
				$this->strings['process_success']     = __( 'Plugin installed successfully.', 'remarky' );
			...
			public function activate_strings() {
				...
				$this->strings['activation_failed']  = __( 'Plugin activation failed.', 'remarky' );
				$this->strings['activation_success'] = __( 'Plugin activated successfully.', 'remarky' );
	...
	if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
		...
		class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
			...
			public function add_strings() {
				...
				if ( TGM_Plugin_Activation::$instance->is_automatic ) {
					$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'remarky' );
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'remarky' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'remarky' ) . '</span><span class="hidden">' . __( 'Hide Details', 'remarky' ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'remarky' );
					$this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'remarky' );
				}
				/** Default installation strings */
				else {
					$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'remarky' );
					$this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'remarky' );
					$this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'remarky' );
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'remarky' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'remarky' ) . '</span><span class="hidden">' . __( 'Hide Details', 'remarky' ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'remarky' );
					$this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'remarky' );
				}
			...
			public function bulk_footer() {
				...
				foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
					if ( ! is_plugin_active( $plugin['file_path'] ) ) {
						echo '<p><a href="' . add_query_arg( 'page', TGM_Plugin_Activation::$instance->menu, admin_url( TGM_Plugin_Activation::$instance->parent_url_slug ) ) . '" title="' . esc_attr( TGM_Plugin_Activation::$instance->strings['return'] ) . '" target="_parent">' . __( TGM_Plugin_Activation::$instance->strings['return'], 'remarky' ) . '</a></p>';
						$complete[] = $plugin;
						break;
					}
					/** Nothing to store */
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'remarky' ) . '">' . __( 'Return to the Dashboard', 'remarky' ) . '</a>' ) . '</p>';
					echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
				}
